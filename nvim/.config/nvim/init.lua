...
local function make_fzf(picker)
  if (nil == picker) then
    _G.error("Missing argument picker on init.fnl:39", 2)
  else
  end
  local function _8_()
...
local function build(ev)
  if (nil == ev) then
    _G.error("Missing argument ev on init.fnl:57", 2)
  else
  end
...
local function gh(repo)
  if (nil == repo) then
    _G.error("Missing argument repo on init.fnl:70", 2)
  else
  end
...
local function make_setup(plugin)
  if (nil == plugin) then
    _G.error("Missing argument plugin on init.fnl:91", 2)
  else
  end
